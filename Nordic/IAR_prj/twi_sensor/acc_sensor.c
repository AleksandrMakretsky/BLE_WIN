...
/* Common addresses definition for temperature senso. */
...
#define LIS3DH_CTRL_REG1       0x20U //буква U означает что число беззнаковае типа unsigned int
...
const uint8_t LIS3DH_register_settings[10][2]={ ///рабочая настройка акселерометра как кнопка singl click
  //от 7.06.19
...
//	nrf_gpio_cfg_input(ACC_INT_PIN, NRF_GPIO_PIN_NOPULL);
...
//		pin = nrf_gpio_pin_read(ACC_INT_PIN);
		
//		if ( nrf_gpio_pin_read(ACC_INT_PIN) != 0 ) {
//			TEST_INV;
//		}
...
//		f_drv_twi_rx(&m_twi, LM75B_ADDR, &m_sample,1);
//		nrf_drv_twi_rx(&m_twi, LM75B_ADDR, &m_sample, sizeof(m_sample));
